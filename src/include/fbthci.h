...
public:
   ...
   virtual DWORD OnEvent(PFBT_HCI_EVENT_HEADER pEvent, DWORD Length);
    ...
    virtual DWORD SendReset();

    virtual DWORD SendInquiry(ULONG LAP, BYTE InquiryLength, BYTE NumResponses);
    virtual DWORD SendInquiryCancel();

    virtual DWORD SendReadBDADDR();

    virtual DWORD SendReadBufferSize();
    ...
    virtual DWORD SendReadLocalName();
	...
	virtual DWORD SendReadLocalVersionInformation();
...
protected:
	...
	virtual DWORD SendHciCommand(const PFBT_HCI_CMD_HEADER lpCommand, DWORD dwBufferSize);
